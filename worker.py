...
def _friendly_error(exc: Exception, context: str) -> str:
    ...
    elif "контроль качества" in lower or "не прошёл" in lower:
        hint = (
            "Перевод остановлен: фрагмент не прошёл автоматический контроль качества. "
            "Непереведённый исходный текст не был сохранён как готовый результат."
        )
    else:
        hint = context
